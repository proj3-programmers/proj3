...
def collect_and_insert_data(endpoint, start_date, end_date):
    ...
    if all_data:
        ...
        insert_query = """
        INSERT INTO dooby99.WH_PRFSTS_AREA_INFO (BASE_DT, AREA_NAME, PRF_PRO_CNT, PRF_DT_CNT, NMRS, AMOUNT)
        VALUES (%s, %s, %s, %s, %s, %s)
        """
...
dag = DAG(
    'kopis_initial_data_load',
    default_args=default_args,
    description='One-time initial data load from KOPIS API and insert into Redshift',
    schedule_interval=None,  # 스케줄 반복을 하지 않도록 설정
)

# Airflow Task용 함수
def collect_data_execution():
    start_date = '20240101'  # 초기 적재 시작 날짜
    end_date = '20240610'    # 초기 적재 종료 날짜
